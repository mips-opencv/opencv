...
struct RGB2HLS_b
{
    ...
    : srccn(_srccn), cvt(3, _blueIdx, (float)_hrange)
    {
        ...
        #elif CV_MSA
        v_scale_inv = msa_dupq_n_f32(1.f/255.f);
    ...
    void operator()(const uchar* src, uchar* dst, int n) const
    {
        ...
        for( i = 0; i < n; i += BLOCK_SIZE, dst += BLOCK_SIZE*3 )
        {
            ...
            for( ; j <= (dn - 16) * 3; j += 48 )
            {
                ...
                v8u16 dst_lo = msa_qpack_u32(msa_cvttintq_u32_f32(v_src0[0]),
                                             msa_cvttintq_u32_f32(v_src1[0]));
                v8u16 dst_hi = msa_qpack_u32(msa_cvttintq_u32_f32(v_src2[0]),
                                             msa_cvttintq_u32_f32(v_src3[0]));
